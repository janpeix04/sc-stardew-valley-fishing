...
export default {
    data() {
        return {
            SCALE_FACTOR,
            ATTEMPTS_DIFFICULTY,
            capturesLeftMargin: CapturesLeftMargin,
            capturesRightMargin: CapturesRightMargin,
...
<template>
    <div id="capturesWrapper">
        <img :src="capturesLeftMargin" alt="Captures left margin">
        <div>
            <img v-for="i in ATTEMPTS_DIFFICULTY.length" class="captureResultWrapper" :src="captureResultWrapper"
                alt="Capture result wrapper">
        </div>
        <div id="capturedFishWrapper">
            <slot></slot>
        </div>
        <img :src="capturesRightMargin" alt="Captures right margin">
    </div>
</template>

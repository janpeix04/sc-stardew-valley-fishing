...
export default {
    name: 'Hud',
    props: {
        showCaughtFishTrigger: { type: Boolean, required: true },
        enableActionButtonTrigger: { type: Boolean, required: true }
    },
    emits: [
        'setPlayerState',
        'setCapturedFish',
        'update:showCaughtFishTrigger',
        'update:enableActionButtonTrigger'
    ],
    components: {
        Minigame,
        BaseCaptures,
    ...
    methods: {
        ...
        handleClick() {
            if (this.isMinigameVisible || !this.enableActionButtonTrigger) return;
        ...
        async handlePressed() {
            if (!this.isMinigameVisible) return;
            await fetch('http://localhost:8081/move_catch_bar_up');
        },
        async handleReleased() {
            if (!this.isMinigameVisible) return;
            await fetch('http://localhost:8081/stop_moving_catch_bar_up');
        }
    }
};
</script>
